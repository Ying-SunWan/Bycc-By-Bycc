...
def show_answer():
    inp_code.configure(state=tk.NORMAL)  # may not need
    out.configure(state=tk.NORMAL)  # may not need
    ...
    for line in split_inp:
        translated_line = translate_to_c(line)
        output_code += (translated_line + '\n')

    out.insert(1.0, output_code)  # send output to out box
    out.configure(state=tk.DISABLED)  # so when translated, the output cannot be edited by user
